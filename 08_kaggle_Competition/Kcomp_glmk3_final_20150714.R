...
set.seed(47)
TrainRF2 = randomForest(sold ~ . - UniqueID , data=Train2, nodesize = 7)
Trainglm3 = glm(sold ~ . - UniqueID - WordCount , data=Train3, family=binomial)
set.seed(47)
TrainRF3 = randomForest(sold ~ . - UniqueID - WordCount, data=Train3, nodesize = 11)
...
binsize = 50
histTrain = hist(AllPredictionsTrain, breaks = binsize)
histTrain.RF = hist(AllPredictionsTrain.RF, breaks = binsize)
histTest = hist(AllTestPred$PredTest, breaks = binsize)
histTest.RF = hist(AllTestPred$PredTest.RF, breaks = binsize)
print(cor(histTrain$density, histTest$density))
print(cor(histTrain.RF$density, histTest.RF$density))
